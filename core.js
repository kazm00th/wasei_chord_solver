(function (root, factory) {
  ...
  function resolveLevel(rootIndex, currentQuality, level, allowDiminished = false, checkLayer = false) {
    ...
    if (level.degree === "napoliII" || level.degree === "raisedVII") {
      ...
      // 『総合和声』実技篇 p.101 注1・注2（p.100-101 の調関係の一覧表）——ナポリII調・変位VII調は
      // それぞれ短調・長調の層にしか現れない。変位VII調は長調に、ナポリII調は短調に、
      // 固有の調が無い位置を埋める調として加えられている。
  ...
  function applyAlteration(chord, form, alteration, diatonicContext, quality, isVofV, isVInMinorKey) {
    ...
    if (alteration.up && isVInMinorKey) {
      // 『総合和声』実技篇 第4章 p.121注1——短調の Ⅴ では、上方変位で作る音（↑Ⅱ）が
      // 主調の固有Ⅲ音に重なってしまい変位にならないので、この形は使わない。
      ...
      throw new ChordError("短調内のV度諸和音には上変を適用できません（『総合和声』p.121 注1 に基づく）");
    }

    // 付加6・付加4への下変は、より具体的な理由（S機能側に下変の節が無い）を
    // 後段で個別に案内するため、ここでは対象外にする。
    if (alteration.down && !isVofV && !(form.add6 || form.add4)) {
      // 『総合和声』実技篇 第5章 p.128-129——下方変位は、節そのものが V̇諸和音
      // （V度のV度族）に対して立てられており、定義も V̇諸和音（短調）の第5音を半音下げる
      // 形に限られている。他の音度に対する下方変位の規定は原典に存在しない。
      // 構造上の理由も p.129 にある——V̇諸和音（短調）の第5音はもとから ↑VI 音
      // （p.125）なので、半音下げれば固有の VI 音に戻る。第5音が↑VI音であるのはV̇諸和音だけで、
    ...
    if (form.add6 || form.add4) {
      // HANDOFF §2 の定義——上変は V度の第5音または IV度付加の第6音を半音上げ、
      // 下変は V度のV度の第5音を半音下げる。下変はIV度付加系には定義されて
      // いない（島岡理論の英訳39-5 p.61-64でも、上変にはD/S両側の鏡像的な節がある
      // 一方、下変はD側のみで、S側（IV度付加）に対応する節は存在しない。下変は
      // 別の調の固有和音として再解釈される現象であり、上変のように単純に対称化
